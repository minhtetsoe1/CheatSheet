        ...
        <thead>
            <tr>
               ...
               <!--  <th scope="col">User</th> -->
                <th scope="col">Delete/Active</th>
            </tr>
        </thead>
        <tbody>
            <c:forEach items="${allCheatSheet}" var="cs" varStatus="index">
                <tr>
                    <td scope="row">${index.count}</td>
                     <td><a href="../cheat-sheet/show/${cs.id }">${cs.title}</a></td>
                    <td>${cs.content}</td>
                    <td>${cs.createdAt}</td>
                   <%--  <td>${cs.user.name}</td> --%>
                    <td>
                        <c:if test="${cs.status != 1}">
                            <a href='<c:url value="/cheat-sheet/active/${cs.id}"/>'><button type="button"
                                    class="btn btn-secondary">Active</button></a>
                        </c:if>
                        <c:if test="${cs.status != 0}">
                            <a href='<c:url value="/cheat-sheet/delete/${cs.id}"/>'><button type="button"
